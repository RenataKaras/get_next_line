...
/*   Updated: 2024/01/12 11:50:12 by rkaras        ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_strjoin(char *line, char *buffer)
{
	...
	totals = ft_strlen(line) + ft_strlen(buffer);
	newstring = ft_calloc (totals + 1, sizeof(char));
	if (!newstring)
	{
		free(line);
		return (NULL);
	}
	ft_strlcpy(newstring, line);
	ft_strlcpy(&newstring[ft_strlen(line)], buffer);
	free(line);
	return (newstring);
}
